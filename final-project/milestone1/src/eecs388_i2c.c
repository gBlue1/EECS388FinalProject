...
void steering(int angle){
    uint8_t low = 0;
    uint8_t high = 0;
    int breakupNum = getServoCycle(angle);

    breakup(breakupNum, &low, &high);
    bufRead[0]=0; //need to change value, dependent on code from steering
...
void driveForward(uint8_t speedFlag){
    ...
    int breakupNum = (2*speedFlag) + 311;
    uint8_t low = 0;
    uint8_t high = 0;
...
void driveReverse(uint8_t speedFlag){
    ...
    int breakupNum = 269 - (2*speedFlag);
    uint8_t low = 0;
    uint8_t high = 0;
...
int main()
{
    set_up_I2C();
    stopMotor();
    steering(-45); // changed for testing purposes by TA
    delay(2000);
    driveForward(2);
    delay(2000);
    steering(45);//changed for testing purposes by TA
